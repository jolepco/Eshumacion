...
class login extends Rest {
    ...
    private function validar($usuario,$pass) {
		...
		if($usuario != '' && $pass != ''){
			...
			$sql = "SELECT id, perfil, username, password, Cargo, IdPersona, id_subred ";
			$sql .= " FROM usuarios  ";
			...
			if(count($filasDB) > 0){
				...
				$resultado['id_usuario'] = $filasDB[0]['id'];
				$resultado['usuario'] = $filasDB[0]['username'];
				$resultado['cargo'] = $filasDB[0]['Cargo'];
				$resultado['id_subred'] = $filasDB[0]['id_subred'];
