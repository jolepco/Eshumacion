...
class validacion_model extends CI_Model {
   ...
   public function rayosx_pendientes($perfil){
        ...
        $cadena_sql = "SELECT RX.id, RX.user, RX.estado, RX.created_at, RX.categoria, RX.visita_previa, RX.fecha_subsanacion1, RX.fecha_subsanacion2, RX.fecha_envio, RF.fecha_estado, RF.observaciones, ET.descripcion descEstado, TI.Descripcion descTipoIden , RX.solicita_prorroga, PE.*
		...
		if($perfil == 3){
			$cadena_sql .= " WHERE RX.estado IN (4,16,25,41)";	
		}else if($perfil == 8){
			$cadena_sql .= " WHERE RX.estado IN (2,14,23,13,22)";	
		}else if($perfil == 9){
			$cadena_sql .= " WHERE RX.estado IN (3,15,24)";	
		}else if($perfil == 11){
			$cadena_sql .= " WHERE RX.estado IN (3,15,24)";	
		}
...
public function tramites_resolucionaclaracion($fechai,$fechaf){
		...
		END as grupotituloequivalente,
		resoti.id_resolucion as noactoadmin,
		DATE_FORMAT(VAL.fecha, '%Y-%m-%d') as fecha_actoadmin
