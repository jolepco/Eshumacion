...
class Login_model extends CI_Model {
    ...
    public function validar_codigoED($codigo_verificacion){
        
        $cadena_sql = " SELECT ED.id_expdrogas_tramite, ED.id_usuario, ED.fecha_registro, PE.nume_identificacion,  CONCAT(PE.p_nombre, ' ', PE.s_nombre, ' ', PE.p_apellido, ' ', PE.s_apellido) as nombre, RE.id_resolucion, RE.fecha_resolucion, ET.descripcion
        FROM expdrogas_tramite ED 
        JOIN persona PE ON PE.id_persona = ED.id_usuario 
        JOIN pr_estado_tramite ET ON ET.id_estado = ED.id_estado 
        JOIN pr_tipoidentificacion TI ON TI.IdTipoIdentificacion = PE.tipo_identificacion 
        JOIN expdrogas_resoluciones RE ON PE.id_persona = RE.id_persona
    ...
    public function validar_codigoSST($codigo_verificacion){
        
        $cadena_sql = " SELECT sst.id_tramite, sst.id_usuario, sst.fecha_creacion, PE.nume_identificacion,  CONCAT(PE.p_nombre, ' ', PE.s_nombre, ' ', PE.p_apellido, ' ', PE.s_apellido) as nombre, RE.id_resolucion, stf.fecha_estado, ET.descripcion
        FROM sst_tramite sst
        JOIN usuarios US ON US.id= sst.id_usuario
	JOIN persona PE ON PE.id_persona  = US.id_persona
        JOIN sst_estados ET ON ET.id_estado = sst.id_estado 
        JOIN sst_tramite_flujo stf ON stf.tramite_id = sst.id_tramite AND stf.id_estado = sst.id_estado 
        JOIN pr_tipoidentificacion TI ON TI.IdTipoIdentificacion = PE.tipo_identificacion 
        JOIN sst_resoluciones RE ON sst.id_tramite = RE.id_tramite
    ...
    public function validar_codigoRXV($codigo_verificacion){

        $cadena_sql = "SELECT rt.id, rt.user, rt.created_at, PE.nume_identificacion,  CONCAT(PE.p_nombre, ' ', PE.s_nombre, ' ', PE.p_apellido, ' ', PE.s_apellido) as nombre, rte.descripcion, res.id_resolucion, rtf.fecha_estado
        FROM xindustrial_tramite rt
        JOIN usuarios US ON US.id= rt.user
	      JOIN persona PE ON PE.id_persona  = US.id_persona
        JOIN xindustrial_estado_tramite rte ON rte.id_estado = rt.estado
        JOIN xindustrial_resoluciones res ON res.id_tramite = rt.id
        JOIN xindustrial_tramite_flujo rtf ON rtf.tramite_id = rt.id AND rt.estado=rtf.id_estado
        JOIN pr_tipoidentificacion TI ON TI.IdTipoIdentificacion = PE.tipo_identificacion

        WHERE res.codigo_verificacion = '".$codigo_verificacion."'";
        //echo $cadena_sql;
        $query = $this->db->query($cadena_sql);
        $result = $query->row();
        return $result;
    ...
    {
        $sql = " SELECT id,	id_persona, perfil, username, password, estado, fecha_terminos, tramites ";
    ...
    public function departamentos_coldesc($dpto){
        $cadena_sql = " SELECT Descripcion FROM pr_departamento WHERE IdPais = 170 AND IdDepartamento = ".$dpto." ";
    ...
    public function municipios_coldesc($muni){
        $cadena_sql = " SELECT Descripcion FROM pr_municipio WHERE IdMunicipio = ".$muni." ";
    ...
    public function tipo_identificacion(){
        $cadena_sql = " SELECT IdTipoIdentificacion, Codigo, Descripcion FROM pr_tipoidentificacion WHERE IdTipoIdentificacion IN(1,2,3,6,7) ";
    ...
    public function existe_representantelegal($tipo_identificacionrl, $numero_identificacionrl){
        $cadena_sql = " SELECT id_persona, tipo_iden_rl, nume_iden_rl FROM persona ";
        $cadena_sql .= " WHERE tipo_iden_rl = '".$tipo_identificacionel."' AND nume_iden_rl = '".$numero_identificacionrl."'";
