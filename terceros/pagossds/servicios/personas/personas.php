...
class personas extends Rest {
    ...
    private function gestorpersonas($usuario) 
	{
		...
		if($usuario != ''){
			...
			//Buscar Usuario
			$sql = "SELECT id, perfil, username ";
			$sql .= " FROM usuarios ";
			$sql .= " WHERE username = '".$usuario."'";
			
			$queryUsuario = $this->_conn->query($sql);
			$resultadoUsuario = $queryUsuario->fetchAll(PDO::FETCH_ASSOC);
			...
			if(count($resultadoUsuario) > 0)
			{
				...
				$sql .= " Edad, FechaNacimiento, Sexo, GrupoSanguineo, FactorRH, IdMunicipioNacimiento, IdLocalidad, Barrio, Direccion, Telefono ";
				$sql .= " FROM Persona PE ";
				$sql .= " JOIN Asignacion ASI ON ASI.IdPersona = PE.IdPersona ";
				$sql .= " WHERE IdGestor = '".$resultadoUsuario[0]['id']."' ";
			...
			$respuesta['estado'] = 'error';
			$respuesta['respuesta'] = 'Debe enviar usuario gestor';
			$this->mostrarRespuesta($this->convertirJson($respuesta), 200);
		}     
    }
