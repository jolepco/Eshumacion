...
$(document).ready(function(){
	...
	$("#encargado_pnombre").bloquearNumeros().convertirMayuscula().bloquearCaracteres().bloquearCaracteres1(); 
	$("#encargado_snombre").bloquearNumeros().convertirMayuscula().bloquearCaracteres().bloquearCaracteres1();
	$("#encargado_papellido").bloquearNumeros().convertirMayuscula().bloquearCaracteres().bloquearCaracteres1();
	$("#encargado_sapellido").bloquearNumeros().convertirMayuscula().bloquearCaracteres().bloquearCaracteres1();
	$("#encargado_numdoc").maxlength(18);
	$("#encargado_telefono").bloquearTexto().maxlength(16);
	$("#encargado_celular").bloquearTexto().maxlength(16);
  ...
  /*if(!$("input.dependencia:checked[type=checkbox]").length>0){  //checked = $("input:checked[type=checkbox]").length;
    ...
    if ($("#form").valid() == true){
        ...
        $.ajax({
          type: "POST",
          url: base_url + "plazas/save_nuevo_tramite",
          data: $("#form").serialize(),
          dataType: "json",
          ...
          success: function(data){
            ...
            if( data.result )//true
            {
              ...
              $("#div_load").css("display", "none");
              $('#btnSubmit').removeAttr('disabled');
              location.href = base_url + "plazas/registrar_tramite";
