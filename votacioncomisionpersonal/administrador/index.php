...
function LogControl($faccion2, $idest2) {
	...
	$resp=mysqli_query($link,sprintf("select id,nombres,apellidos,enviarcontrasena from administradores where usuario=%s and password=%s",comillas($_POST['usuario']),comillas($clave)));
	if ($row= mysqli_fetch_array($resp)) {
		...
		echo '<h2>BIENVENIDO(A): '.utf8_encode($row['nombres']).' '.utf8_encode($row['apellidos']).'</h2>';
		...
		if ($row['id']!=1) {
			...
			echo '<tr><td><a href="javascript:NuevaVentana(\'general1.php\')" title="Consulta de servidores por dependencia<"><img src="../iconos/folder.png" border="0" alt="Folder" /> Consulta de servidores por dependencia</a></td></tr>';
		...
		if ($row['id']==1) {
            ...
            echo '<tr><td><a href="javascript:NuevaVentana(\'configuraciones.php\')" title="Activar o Desactivar votación virtual"><img src="../iconos/hoja.png" border="0" alt="Config" /> Activar o Desactivar votación virtual</a></td></tr>';
