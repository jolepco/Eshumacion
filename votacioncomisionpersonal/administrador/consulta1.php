...
if ($_COOKIE['VotaDatAdmin']==1) {
	//****Agregar nuevo servidor*******
		if (isset($_POST['envia_estudiante'])) {
			if ((borra_espacios($_POST['nom_est'])!="")and(borra_espacios($_POST['ape_est'])!="")and(borra_espacios($_POST['doc_est'])!="")) {
				...
				$fnom_est=utf8_decode($fnom_est);
				$fape_est=cambia_mayuscula(borra_espacios($_POST['ape_est']));
				$fape_est=utf8_decode($fape_est);
				$fdoc_est=borra_espacios($_POST['doc_est']);			
				$fclave_est=md5($_POST['clave_est']);
				$fcor_est=borra_espacios($_POST['fcor_est']);
				if (filter_var($fcor_est, FILTER_VALIDATE_EMAIL)) {
					$fcor_est=borra_espacios($_POST['fcor_est']);
				}
				else {
					include_once("encabezado.html");
					print "<strong>El correo es inválido<br />";
			...
			$cons_sql  = sprintf("INSERT INTO estudiantes(grado,nombres,apellidos,documento,clave,correo) VALUES(%d,%s,%s,%s,%s)", $fgrado, comillas($fnom_est),comillas($fape_est),comillas($fdoc_est),comillas($fclave_est),comillas($fcor_est));
	...
	mysqli_query($link,$cons_sql5);
		...
		if (isset($_POST['edita_est'])) {
			...
			//****Actualizar en la BD*******
			$cons_sql3  = sprintf("UPDATE estudiantes SET grado=%d, nombres=%s, apellidos=%s, documento=%s, clave=%s, correo=%s WHERE id=%d", $fgrado, comillas($fnom_est), comillas($fape_est), comillas($fdoc_est), comillas($fclave_est),comillas($fcor_est), $_POST['identificador']);
	...
	mysqli_query($link,$cons_sql5);
		...
		if((isset($_GET['agrega']))and($_GET['agrega']=="ok")) { 
			echo '<form name="addest" action="consulta.php?id='.$_GET['id'].'" method="post">';
				...
				echo '</td></tr>';
				echo '<tr>';
				echo '<td style="text-align:right;"><label for="fcor_est">';
				echo '<strong>Correo:</strong>';
				echo '</label></td>';
				echo '<td><input type="text" name="fcor_est" size="30" maxlength="200" title="Escriba el correo electrónico" />';
				echo '</td></tr>';
				echo '<tr>';
		...
		if((isset($_GET['id'])) and (isset($_GET['est'])) and (isset($_GET['editar'])) and ($_GET['editar']=="ok")) { 
			$resp4=mysqli_query($link,sprintf("select * from estudiantes where md5(id)=%s",comillas($_GET['est'])));
				...
				echo '<br /><form name="editaest" action="consulta.php?id='.$_GET['id'].'" method="post">';
					...
					echo '<strong>Nombres:</strong>';
					echo '</label></td>';
					echo '<td><input type="text" name="nom_est" value="'.utf8_encode($row4['nombres']).'" size="30" maxlength="50" title="Escriba los nombres del servidor" />';
					echo '</td></tr>';
					echo '<tr>';
					echo '<td style="text-align:right;"><label for="ape_est">';
					echo '<strong>Apellidos:</strong>';
					echo '</label></td>';
					echo '<td><input type="text" name="ape_est" value="'.utf8_encode($row4['apellidos']).'" size="30" maxlength="50" title="Escriba los apellidos del servidor" />';
					echo '</td></tr>';
					echo '<tr>';
					echo '<td style="text-align:right;"><label for="doc_est">';
					echo '<strong>Documento:</strong>';
					echo '</label></td>';
					echo '<td><input type="text" name="doc_est" value="'.utf8_encode($row4['documento']).'" size="30" maxlength="50" title="Escriba el documento del servidor" />';
					echo '</td></tr>';
					echo '<tr>';
					echo '<td style="text-align:right;"><label for="clave_est">';
					echo '<strong>Contraseña:</strong>';
					...
					echo '<td><input type="password" name="clave_est" size="30" maxlength="50" title="Escriba la contraseña de acceso del servidor" />';
					echo '</td></tr>';
				echo '<input type="hidden" name="identificador" value="'.utf8_encode($row4['id']).'" />';
			...
			$resp5=mysqli_query($link,sprintf("select * from estudiantes where md5(id)=%s",comillas($_GET['est'])));
				if ($row5 = mysqli_fetch_array($resp5)) {
                    ...
                    if (!$row9 = mysqli_fetch_array($resp9)) {
	        			...
	        			echo '¿Desea borrar el servidor '.utf8_encode($row5['apellidos']).' '.utf8_encode($row5['nombres']).' del sistema? ';
                        ...
                        echo '<br /><strong>Advertencia: No puede borrar al servidor '.utf8_encode($row5['nombres']).' '.utf8_encode($row5['apellidos']).', ya que tiene su voto registrado.</strong>';
                    }
			}
			else {
				echo '<table>';
	...
	if($row3 = mysqli_fetch_array($resp3)) {
		...
		echo '<h2>DEPENDENCIA: '.utf8_encode($row3['grado']).'</h2>';
		echo '<div align="center">';
		echo '<div class="enlace">';
		echo '<a href="consulta1.php?id='.$_GET['id'].'&votos=si" title="Consultar lista de servidores que han votado">Ver servidores con voto>></a><br />';
		echo '<a href="consulta1.php?id='.$_GET['id'].'&votos=no" title="Consultar lista de servidores que no han votado">Ver servidores sin voto>></a>';
		echo '</div>';
		if (!isset($_GET['votos'])) {
			echo '<table>';
			if ($_COOKIE['VotaDatAdmin']==1) {
			...
			while($row = mysqli_fetch_array($resp)) {
				echo '<tr>';
				echo '<td>'.utf8_encode($row['apellidos']).' '.utf8_encode($row['nombres']).'</td>';
		...
		echo '<div class="cen">';
		echo '<a href="general1.php" title="consulta por Dependencia"><strong> Volver a consulta por Dependencia</strong></a>';
		echo '</div><br />';
	}
	else {
